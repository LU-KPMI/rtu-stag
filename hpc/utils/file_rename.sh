...
for i in $(awk -F "," '{OFS=","; print $10, $11}' $REGEX); do # Read csv file.
  ...
  then
    ...
    find . -type f \( "${arr_r1[@]}" -name "*.dummy" \) | sort | xargs pv | cat > "$OUTPUTDIR/${newname}_1.fq.gz" | echo "$OUTPUTDIR/${newname}_1.fq.gz" # rename R1 file combining seq and csv variables.
    unset arr_r1
    # merge and rename R2
    for b in $(echo $barcodes | sed "s/,/ /g")
    do
    ...
    find . -type f \( "${arr_r2[@]}" -name "*.dummy" \) | sort | xargs pv | cat > "$OUTPUTDIR/${newname}_2.fq.gz" | echo "$OUTPUTDIR/${newname}_2.fq.gz" # rename R2 file combining seq and csv variables.
    unset arr_r2
    ...
    find . -type f -name "${oldname2}_1.fq.gz" | xargs pv | cat > "$OUTPUTDIR/${newname2}_1.fq.gz" | echo "$OUTPUTDIR/${newname2}_1.fq.gz"
    # rename R2, no merging
    find . -type f -name "${oldname2}_2.fq.gz" | xargs pv | cat > "$OUTPUTDIR/${newname2}_2.fq.gz" | echo "$OUTPUTDIR/${newname2}_2.fq.gz"
  fi
done
